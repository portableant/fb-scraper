...
file_id = config.get('facebook', 'file_id')
...
def scrapeFacebookPageFeedComments(page_id, access_token):
    with open('csv/%s_facebook_comments.csv' % file_id, 'wb') as file:
        w = csv.writer(file)
        ...
        with open('csv/%s_facebook_statuses.csv' % file_id, 'rb') as csvfile:
